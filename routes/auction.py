...
from db.models import AuctionReq, UserPydantic, ItemPydantic, AucServe, AucServeUpdate, AuctionUpdate
...
async def createAuction(data: AuctionReq, userData: Annotated[UserPydantic, Depends(get_current_user)]):
    to_auc = ItemPydantic(id=getattr(data, 'item_id', None), name=data.name, description=data.description, price=data.price, category=data.category, condition=data.condition)
    response = await create_auction(to_auc, auc_serve=AucServe(starting_time=data.starting_time, ending_time=data.ending_time), userData=userData)
...
async def updateAuction(data: AuctionUpdate, userData: Annotated[UserPydantic, Depends(get_current_user)]):
    to_upd = ItemPydantic(id=data.item_id, name=data.name, description=data.description, price=data.price, category=data.category, condition=data.condition)
    try:
